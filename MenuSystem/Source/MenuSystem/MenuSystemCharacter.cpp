...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		if (World)
		{
			World->ServerTravel(FString("/Game/ThirdPerson/Maps/Lobby?listen"));		// Open the level as a listen Server
		}
	}
	else
	{
...
void AMenuSystemCharacter::OnFindSessionComplete(bool bWasSuccessful)
{
	...
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		if (MatchType == FString("FreeForAll"))
		{
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan, FString::Printf(TEXT("Join Match Type: %s"), *MatchType));
			}
